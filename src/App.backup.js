...
class TransposeForm extends React.Component {
  ...
  handleSubmit(event) {
    alert('Your favorite key is: ' + this.state.value);
...
function App() {
  ...
  var transposed = lyricsT.transpose(3)
        ...
        <Form>
          <Form.Group>
            <select onChange={onChangeKey}>
              {[...Array(13).keys()].map(ii => {
                return (
                  <option value={tkeys[ii]} > {tkeys[ii]} </option>
                  //<option value={tkeys[ii]} { skey == ii ? "selected" : "" }> {tkeys[ii]} </option>
                )
              })}
            </select>
          </Form.Group>
        </Form>
        <TransposeForm keys={tkeys}>
        </TransposeForm>
        <Form>
          <Form.Group>
